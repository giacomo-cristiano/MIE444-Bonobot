...
bool enqueue(Queue &q, String element) {
    if (q.count == QUEUE_MAX_SIZE) {
        ...
        Serial.println("Queue is full, dropping oldest command.");
        dequeue(q); // Remove the oldest element
    }
    q.data[q.rear] = element;
    q.rear = (q.rear + 1) % QUEUE_MAX_SIZE;
    q.count++;
...
int enB = 10, in3 = 11, in4 = 12; // LEFT

// Sensor data placeholders
float leftDistance = 0;
float rightDistance = 0;
float Min_Distance = 0;
const float Single_Wall_Threshold = 3.0; // Target distance from wall
// const float alignmentThreshold = 0; // Alignment error threshold in inches, prev. 0.5 
const float kp_alignment = 10;     //   // PID tuning parameter for alignment
const float kd_alignment = 0.32;
const float ki_alignment = 0.25; // old was 0.25 
// PWM limits for motors
const int FMIN_PWM_RIGHT = 170, FMAX_PWM_RIGHT = 210;
const int FMIN_PWM_LEFT = 180, FMAX_PWM_LEFT = 225; // befpre it was 170 and 200 // next chang drop by 20
...
//QUEUE RESETTING LOGIC 
unsigned long lastResetTime = 0; // Track the last reset time
const unsigned long resetInterval = 20000; // Reset every 20 seconds

void resetQueue(Queue &q) {
    for (int i = 0; i < QUEUE_MAX_SIZE; i++) {
        q.data[i] = ""; // Clear all elements
    }
    q.front = 0;
    q.rear = 0;
    q.count = 0;
...
void setup() {
    ...
    pinMode(in3, OUTPUT);
    pinMode(in4, OUTPUT);
    pinMode(7, INPUT);    // Trigger frmo killswitch
...
void executeCommand(String cmd) {
    ...
    //// ADD other if statements for R0, B0, S0
        ...
        targetAngle = cmd.substring(3).toDouble();
        Current_Step = "r0";
    }
...
float LeftalignmentError = 0;
float RightalignmentError = 0;
// REV 1 ONLY LEFT WORKS
void executeDriveForward() {
    int pwmRight = 0;
    int pwmLeft = 0;

    if ((millis() - commandStartTime < driveDuration) && digitalRead(7) == LOW) {
        // Determine alignment error based on the closer wall
        // if (leftDistance < rightDistance) { // Follow left wall
        //     alignmentError = leftDistance - Single_Wall_Threshold;
        // } else { // Follow right wall
        //     alignmentError = rightDistance - Single_Wall_Threshold;
        // }
        ...
        LeftalignmentError = leftDistance - Single_Wall_Threshold;
        RightalignmentError = rightDistance - Single_Wall_Threshold;
        ...
        // if (LeftalignmentIntegral > 4 || RightalignmentIntegral > 4) {
        //     LeftalignmentIntegral = 0;
        //     RightalignmentIntegral = 0;
        // }
        ...
        if ((leftDistance - rightDistance) < 0) { // closer to left wall and minimum is left + alignment threshold 
            // Following left wall

            if (LeftalignmentError > 0) { // Veer left  Too far from wall, shift left (correction is +ve)
        ...
        else { // Following right wall and right distance  
        ...
        delay(50);
        isExecutingCommand = false;
        previousLeftAlignmentError = 0;
        previousRightAlignmentError = 0;
        LeftalignmentIntegral = 0;
        RightalignmentIntegral = 0;
        wdt_enable(WDTO_15MS);  // Set the watchdog timer to reset in 15ms
        while (true)  {
        }
    }
}

float Rotate_Count = 0;
float targetRoll = 0;
float initialRoll = 0;
void executeDriveRotate() {
    ...
    if (Rotate_Count == 0)  {
      ...
      readGyroData();
      initialRoll = roll;
      targetRoll = initialRoll + targetAngle;
      Rotate_Count += 1;
    }

    // Rotate the robot
    if (abs(targetRoll - roll) > threshold) {
        ...
        // if (deltaTime > 0) {
        //     RotationDerivative = (error - previouserror) / deltaTime;
        // }
...
void readGyroData() {
    ...
    accAngleX = (atan(AccY / sqrt(pow(AccX, 2) + pow(AccZ, 2))) * 180 / PI) - 2.96;
