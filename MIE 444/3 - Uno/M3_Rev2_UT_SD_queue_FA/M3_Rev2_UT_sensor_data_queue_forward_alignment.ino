...
const float kp_alignment = 1.25;       // PID tuning parameter for alignment
const float kd_alignment = 0;
...
float Single_Wall_Threshold = 2.0;
...
void executeDriveForward() {
    ...
    if (millis() - commandStartTime < driveDuration) {
        // float alignmentError = leftDistance - rightDistance;
        ...
        float alignmentIntegral = 0;
        if (deltaTime > 0) {
            alignmentDerivative = (alignmentError - previousAlignmentError) / deltaTime;
            alignmentIntegral = (alignmentError - previousAlignmentError) * deltaTime;
        }

        float alignmentCorrection = 0;
        if (abs(alignmentError) > 0) {
            alignmentCorrection = kp_alignment * alignmentError + kd_alignment * alignmentDerivative + ki_alignment * alignmentIntegral;
        }

        int pwmRight = FMAX_PWM_RIGHT, pwmLeft = FMAX_PWM_LEFT;
        int pwmRightmid = (FMAX_PWM_RIGHT + FMIN_PWM_RIGHT) / 2;
        int pwmLeftmid = (FMAX_PWM_LEFT + FMIN_PWM_LEFT) / 2;

        
        if (leftDistance - rightDistance < 0) {
          if (alignmentError < 0) { // Veer left
              pwmRight = constrain((pwmRightmid + alignmentCorrection)*0, FMIN_PWM_RIGHT, FMAX_PWM_RIGHT);
              pwmLeft = constrain((pwmLeftmid + alignmentCorrection), FMIN_PWM_LEFT, FMAX_PWM_LEFT);
          } else if (alignmentError > 0) { // Veer right
              // pwmRight = constrain(pwmRightmid, FMIN_PWM_RIGHT, FMAX_PWM_RIGHT);
              pwmRight = constrain((pwmRightmid  + alignmentCorrection) , FMIN_PWM_RIGHT, FMAX_PWM_RIGHT);            
              pwmLeft = constrain((pwmLeftmid + alignmentCorrection)*0, FMIN_PWM_LEFT, FMAX_PWM_LEFT);
              //  pwmLeft = constrain(pwmLeftmid, FMIN_PWM_LEFT, FMAX_PWM_LEFT);
          }}
